...
Este script es útil para importaciones recurrentes cuando solo cambian los precios
...
def actualizar_precios_medicamentos(excel_path="Data/Alfabeta_Febrero.xlsx"):
    ...
    2. Busca el medicamento por número de registro
    ...
    try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    if existing:
                        ...
                        print(f"⚠️ Fila {idx+2}: Medicamento '{numero_registro}' no encontrado en BD")
        ...
        # Resumen
        print("\n" + "="*50)
        print("📊 RESUMEN DE ACTUALIZACIÓN DE PRECIOS")
        print("="*50)
        print(f"✓ Actualizados: {total_actualizados}")
        if total_no_encontrados > 0:
            print(f"⚠️ No encontrados: {total_no_encontrados}")
        if total_errores > 0:
