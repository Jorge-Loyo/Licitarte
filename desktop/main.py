...
# Agregar raíz al path para acceder a shared
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))

from shared.database.db_manager import DatabaseManager
from src.modules.dashboard import ModuloDashboard
from src.modules.ingreso import ModuloIngreso
from src.modules.gestion import ModuloGestion
from src.modules.ayuda import VentanaAyuda
...
class LicitarteApp(ctk.CTk):
    def __init__(self):
        ...
        logo_path = Path(__file__).parent / "assets" / "Img" / "Logo_licitarte.png"
        if logo_path.exists():
            self.iconbitmap(default=str(logo_path))
        ...
        logo_path = Path(__file__).parent / "assets" / "Img" / "Logo_licitarte.png"
        if logo_path.exists():
            logo_img = ctk.CTkImage(Image.open(str(logo_path)), size=(80, 80))
