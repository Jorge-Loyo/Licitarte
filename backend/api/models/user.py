...
class User(UserMixin):
    def __init__(self, id, username, email, is_active=True, role='admin'):
        self.id = id
    ...
    def get(user_id):
        ...
        with db.get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row:
                return User(row[0], row[1], row[2], row[3], role='admin')
        return None
    ...
    def get_by_username(username):
        ...
        with db.get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row:
                user = User(row[0], row[1], row[2], row[3], role='admin')
