...
from backend.constants import EstadoProducto, ProductoCotizar
...
class LicitacionCreate(BaseModel):
    ...
    porcentaje_poliza: Optional[float] = None
...
class ProductoCreate(BaseModel):
    ...
    resultado: str = Field(default=EstadoProducto.PARCIAL)
    ...
    observaciones: Optional[str] = None
    producto_cotizar: str = Field(default=ProductoCotizar.PRINCIPAL, max_length=50)
    alternativas: List[dict] = Field(default_factory=list)
    
    @validator('resultado')
    def resultado_valido(cls, v):
        if v not in EstadoProducto.valores():
            raise ValueError(f'Resultado debe ser: {', '.join(EstadoProducto.valores())}')
        return v
    
    @validator('producto_cotizar')
    def producto_cotizar_valido(cls, v):
        if v not in ProductoCotizar.valores():
            raise ValueError(f'Producto cotizar debe ser: {', '.join(ProductoCotizar.valores())}')
        return v
