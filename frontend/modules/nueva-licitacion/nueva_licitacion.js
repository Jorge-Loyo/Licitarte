...
async function agregarProducto() {
  ...
  productoItem.querySelector('.btn-agregar-alternativa').onclick = () => agregarAlternativa(currentCount);
  productoItem.querySelector('.producto-seleccionado').onchange = () => recalcularTotales();
...
async function buscarMonodroga(input) {
  ...
  try {
    ...
    sugerencias.innerHTML = monodrogas
      .map(
        (m) =>
...
async function seleccionarMonodroga(element, monodroga) {
  ...
  const sugerencias = container.querySelector(".monodroga-sugerencias");
  const laboratorioInput = container.querySelector(
    ".producto-laboratorio-input",
  );
...
async function buscarLaboratorio(input) {
  ...
  const selectorMarcaPresentacion = container.querySelector(
    ".producto-selector-marca-presentacion",
  );
  ...
  if (texto.length === 0) {
    sugerencias.style.display = "none";
    selectorMarcaPresentacion.innerHTML =
  ...
  try {
    ...
    sugerencias.innerHTML = laboratorios
      .map(
        (lab) =>
...
function manejarCambioResultado(select) {
  ...
  const motivoPerdidaContainer = container.querySelector(
    ".motivo-perdida-container",
  );
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (existe) {
      mostrarMensaje(
        "Ya existe una licitación con este número para el cliente seleccionado",
        "error",
      );
      return;
    }
...
async function nuevoOferente(btn) {
  ...
  quickAddTipo = "oferente";
  document.getElementById("quickAddTitulo").textContent = "Nuevo Oferente";
  ...
  document.getElementById("quickAddForm").onsubmit = async (e) => {
    e.preventDefault();
    const nombreOferente = document.getElementById("quickAddNombre").value;
    if (!nombreOferente) return;

    try {
      const response = await fetch("/api/oferentes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre: nombreOferente }),
      });
      const result = await response.json();
      if (result.success) {
        ...
        select.innerHTML =
          ...
          oferentes
            .map((o) => `<option value="${o.nombre}">${o.nombre}</option>`)
            .join("");
        select.value = nombreOferente;
        cerrarModalQuickAdd();
...
async function nuevaMarca(btn) {
  quickAddTipo = "marca";
  document.getElementById("quickAddTitulo").textContent = "Nueva Marca";
  document.getElementById("quickAddLabel").textContent = "Nombre *";
  document.getElementById("quickAddNombre").value = "";
  document.getElementById("modalQuickAdd").style.display = "block";
  document.getElementById("quickAddForm").onsubmit = async (e) => {
    e.preventDefault();
    const nombreMarca = document.getElementById("quickAddNombre").value;
...
document
  .getElementById("nuevoClienteForm")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    const data = {
      ...
      organismo_jurisdiccion: document.getElementById("nuevoClienteOrganismo")
        .value,
    };
...
function abrirModalNuevoTipo() {
  quickAddTipo = "tipo";
  document.getElementById("quickAddTitulo").textContent =
...
document.addEventListener("click", (e) => {
  if (
    !e.target.classList.contains("producto-monodroga-input") &&
    !e.target.classList.contains("producto-laboratorio-input") &&
    !e.target.classList.contains("alt-laboratorio-input")
  ) {
    document
      .querySelectorAll(".monodroga-sugerencias")
      .forEach((s) => (s.style.display = "none"));
    document
      .querySelectorAll(".laboratorio-sugerencias")
      .forEach((s) => (s.style.display = "none"));
    document
      .querySelectorAll(".alt-laboratorio-sugerencias")
      .forEach((s) => (s.style.display = "none"));
  }
});
...
function validarNumeroRenglon(input) {
  ...
  productos.forEach((item) => {
    if (item !== container) {
      const otroNumero = item
        .querySelector(".producto-numero-renglon")
        .value.trim();
...
function actualizarListaPliegos() {
  ...
  lista.innerHTML = archivosPliegosSeleccionados
    .map(
      (archivo, index) => `
...
async function agregarAlternativa(productoId) {
...
async function buscarLaboratorioAlternativa(input, monodroga) {
  ...
  try {
    const params = new URLSearchParams({ monodroga: monodroga, q: texto });
    const url = new URL("/api/laboratorios/buscar", window.location.origin);
...
async function verificarLicitacionExistente() {
  ...
  try {
    const response = await fetch(
      `/api/licitaciones/verificar?numero=${encodeURIComponent(numero)}&cliente_id=${clienteId}`,
    );
    const result = await response.json();
...
async function generarVistaPrevia() {
  ...
  if (!numeroLicitacion || !clienteSelect.value || !fecha) {
    mostrarMensaje(
      "Complete los datos básicos: Número de licitación, Cliente y Fecha",
      "error",
    );
    return;
  }
  ...
  if (productosData.length === 0) {
    mostrarMensaje("Debe agregar al menos un producto completo", "error");
    return;
  }
  ...
  productosData.sort((a, b) => {
    ...
    if (
      a.numeroRenglonDisplay.includes("ALT") &&
      !b.numeroRenglonDisplay.includes("ALT")
    )
      return 1;
    if (
      !a.numeroRenglonDisplay.includes("ALT") &&
  ...
  const total = productosData
    .filter((p) => p.esSeleccionado)
