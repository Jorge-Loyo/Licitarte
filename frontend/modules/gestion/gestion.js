...
function formatearMoneda(valor) {
  if (valor >= 1000000) {
    return (
      "$" +
      (valor / 1000000).toLocaleString("es-AR", {
...
function mostrarLicitaciones() {
  ...
  licitacionesPagina.forEach((l) => {
    ...
    // Create cells with proper text content to prevent code injection
    const tdNumero = document.createElement("td");
    tdNumero.textContent = l.numero;
    tr.appendChild(tdNumero);

    const tdCliente = document.createElement("td");
    tdCliente.textContent = l.cliente;
    tr.appendChild(tdCliente);

    const tdTipo = document.createElement("td");
    tdTipo.textContent = l.tipo_licitacion;
    tr.appendChild(tdTipo);

    const tdFecha = document.createElement("td");
    tdFecha.textContent = l.fecha;
    tr.appendChild(tdFecha);

    const tdTotal = document.createElement("td");
    tdTotal.textContent = formatearMoneda(totalCotizado);
    tr.appendChild(tdTotal);

    const tdPresupuesto = document.createElement("td");
    if (numeroPresupuesto !== "-") {
      tdPresupuesto.textContent = numeroPresupuesto;
      const btn = document.createElement("button");
      btn.className = "btn-primary";
      btn.style.cssText =
        "padding: 4px 8px; font-size: 12px; margin-left: 5px;";
      btn.textContent = "👁️";
      btn.title = "Ver presupuesto";
      btn.onclick = () =>
        window.open(`/presupuesto/${numeroPresupuesto}`, "_blank");
      tdPresupuesto.appendChild(btn);
    } else {
      tdPresupuesto.textContent = "-";
    }
    ...
    btnVer.style.margin = "2px";
    btnVer.textContent = "👁️ Ver";
    btnVer.title = "Ver detalle de productos de esta licitación";
    btnVer.onclick = () => verDetalle(l.id);
    tdAcciones.appendChild(btnVer);

    const btnEditar = document.createElement("button");
    btnEditar.className = "btn-success";
    btnEditar.style.margin = "2px";
    btnEditar.textContent = "✏️ Editar";
    btnEditar.title = "Editar datos generales de la licitación";
    btnEditar.onclick = () => editarLicitacionPagina(l.id);
    tdAcciones.appendChild(btnEditar);

    const btnExcel = document.createElement("button");
    btnExcel.className = "btn-primary";
    btnExcel.style.margin = "2px";
    btnExcel.textContent = "📊 Excel";
    btnExcel.title = "Exportar licitación a archivo Excel";
    btnExcel.onclick = () => exportarExcel(l.id);
    tdAcciones.appendChild(btnExcel);

    const btnPresupuesto = document.createElement("button");
    btnPresupuesto.className = "btn-primary";
    btnPresupuesto.style.margin = "2px";
    btnPresupuesto.textContent = "📄 Nuevo Presupuesto";
    btnPresupuesto.title = "Generar presupuesto PDF de esta licitación";
    btnPresupuesto.onclick = () => generarPresupuesto(l.id);
    tdAcciones.appendChild(btnPresupuesto);

    const btnEliminar = document.createElement("button");
    btnEliminar.className = "btn-danger";
    btnEliminar.style.margin = "2px";
    btnEliminar.textContent = "🗑️ Eliminar";
    btnEliminar.title = "Eliminar licitación y todos sus productos";
    btnEliminar.onclick = () => eliminar(l.id);
  ...
  const totalPaginas = Math.ceil(
    licitacionesFiltradas.length / porPaginaGestion,
  );
  document.getElementById("paginaInfoGestion").textContent =
    `Página ${paginaGestion} de ${totalPaginas || 1}`;
  document.getElementById("btnAnteriorGestion").disabled = paginaGestion === 1;
  document.getElementById("btnSiguienteGestion").disabled =
    paginaGestion >= totalPaginas;
}
...
async function verDetalle(id) {
  ...
  // Validate id is a number to prevent SSRF
  if (!Number.isInteger(id) || id <= 0) {
    console.error("Invalid licitacion ID");
    return;
  }
  ...
  document.getElementById("detalleTitle").textContent =
    `Licitación N° ${licitacion.numero}`;
  ...
  for (const p of productos) {
    ...
    if (p.producto_cotizar === "principal" || !p.producto_cotizar) {
      if (p.resultado === "Adjudicado") {
        ...
        const prodCatalogo = catalogo.find(
          (c) =>
            ...
            c.marca?.toLowerCase().trim() ===
              marcaActual?.toLowerCase().trim() &&
            c.presentacion?.toLowerCase().trim() ===
              presentacionActual?.toLowerCase().trim(),
        );
        ...
        precioGanador = p.precio_ganador
          ? formatearMoneda(p.precio_ganador)
          : "-";
        try {
    ...
    if (alternativas && alternativas.length > 0) {
      for (let altIndex = 0; altIndex < alternativas.length; altIndex++) {
        ...
        const esAlternativaSeleccionada =
          p.producto_cotizar === `alt-${p.id}-${altIndex}`;
        ...
        if (esAlternativaSeleccionada) {
          if (p.resultado === "Adjudicado") {
            ...
            precioGanadorAlt = p.precio_ganador
              ? formatearMoneda(p.precio_ganador)
              : "-";
            try {
              const ofertasResponse = await fetch(`/api/ofertas/${p.id}`);
...
function editarProducto(id, producto) {
  ...
  document.getElementById("editMarcaGanadora").value =
    producto.marca_ganadora || "";
  document.getElementById("editPrecioGanador").value =
    producto.precio_ganador || "";
  document.getElementById("editMarcaOfrecida").value =
    producto.marca_ofrecida || "";
  document.getElementById("editMotivoPerdida").value =
    producto.motivo_perdida || "";

  // Obtener costo unitario del producto
  const prodCatalogo = catalogo.find(
    (p) =>
      p.monodroga === producto.monodroga &&
  ...
  document
    .getElementById("editPrecio")
    .addEventListener("input", manejarCambioResultadoEdicion);
  document
    .getElementById("editPrecioGanador")
    .addEventListener("input", manejarCambioResultadoEdicion);
  document
    .getElementById("editResultado")
    .addEventListener("change", manejarCambioResultadoEdicion);
...
function manejarCambioResultadoEdicion() {
  ...
  if (resultado === "Adjudicado") {
    ...
    motivosPerdida.forEach((m) => {
      ...
      motivoPerdidaSelect.appendChild(option);
    });
    ...
    if (pGanador > 0 && pOfertado > 0) {
      ...
      document.getElementById("diferenciaPesos").textContent =
        formatearMoneda(difPesos);
      document.getElementById("diferenciaPorcentaje").textContent =
        difPorcentaje.toFixed(2) + "%";
    } else {
      diferenciasContainer.style.display = "none";
    }
  } else {
...
document.getElementById("editarForm").addEventListener("submit", async (e) => {
  ...
  if (result.success) {
    mostrarNotificacion(
      "✓ Éxito",
      modoEdicion === "crear" ? "Producto agregado" : "Producto actualizado",
    );
    cerrarModalEditar();
...
async function confirmarEliminacion() {
  ...
  const response = await fetch(`/api/licitaciones/${licitacionAEliminar}`, {
    method: "DELETE",
  });
...
async function generarPresupuesto(licitacionId) {
  try {
    ...
    if (result.success) {
      mostrarNotificacion(
        "✓ Éxito",
        `Presupuesto N° ${result.numero} generado correctamente`,
      );
    ...
    mostrarNotificacion(
      "✗ Error",
      "Error al generar presupuesto: " + error.message,
    );
  }
}
...
async function exportarExcel(licitacionId) {
  try {
    // Validate licitacionId is a positive integer to prevent SSRF
    if (!Number.isInteger(licitacionId) || licitacionId <= 0) {
      mostrarNotificacion("✗ Error", "ID de licitación inválido");
      return;
    }

    const encodedId = encodeURIComponent(licitacionId.toString());
    const response = await fetch(
      `/api/licitaciones/${encodedId}/exportar-excel`,
    );
