...
async function subirExcel() {
  ...
  mostrarModalProgreso("Iniciando...", 0, 100);
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          if (data.error) {
            cerrarModalProgreso();
            mostrarModalMensaje("Error al cargar catálogo", data.error, "error");
            return;
          }
          ...
          if (data.done) {
            ...
            mostrarModalMensaje(
              ...
              `Se procesaron ${data.total} productos correctamente`,
              "success"
            );
            cargarCatalogo();
          } else {
            actualizarModalProgreso(data.message, data.progress, data.total);
          }
        }
      }
    }
    ...
    cerrarModalProgreso();
    mostrarModalMensaje("Error", "Error al cargar archivo: " + error.message, "error");
  }

  fileInput.value = "";
}

function mostrarModalProgreso(mensaje, progreso, total) {
...
function actualizarModalProgreso(mensaje, progreso, total) {
  document.getElementById("mensajeProgreso").textContent = mensaje;
  document.getElementById("barraProgreso").style.width = `${(progreso / total) * 100}%`;
  document.getElementById("porcentajeProgreso").textContent = `${progreso} / ${total}`;
}
